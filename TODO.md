...
- [x] ピストンで動かせる不具合を治す
- [x] 爆発でブロックと頭が壊れる＆ドロップする不具合を治す
- [x] configでWorldGuardフラグを設定できるようにする (保護の種類ごとに設定)
- [x] ログイン時に保護ブロックを配る際に、すでにアイテムがスロットに有る場合、ドロップするようにする
- [x] 死んだとき、保護ブロックを復活させるのは、すでに保護しているチャンクがない場合のみとする
- [x] 保護ブロックの頭に水をかけても、頭が壊れないようにする
